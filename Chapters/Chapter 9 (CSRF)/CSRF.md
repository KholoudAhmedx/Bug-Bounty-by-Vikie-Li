...
1. Leak sensitive user information</br>
...
2. Create stored self-XSS by using CSRF</br>
...
A request like this will succeed (we bypassed CSRF protection by removing the CSRF tokens)</br>![image](https://github.com/user-attachments/assets/e188d3eb-605b-4ab5-9dd1-b2bb9dd5c3e1)</br>![image](https://github.com/user-attachments/assets/0e6a1fa3-75c8-4d31-a248-a3221b2f7f86) </br>
3. Take over user accounts by using CSRF</br>
   Account takeover occurs when a CSRF vulnerability exists in a critical functionality, like the code that creates the password, changes the password, resets the password and the like. If the attacker could change the password of a user, then he can take over his account.

# Delivering the CSRF payload
Quite often in bug bounty reports, you’ll need to show companies that attackers can reliably deliver a CSRF payload. How?
1. Trick users into visiting an external malicious site
2. Embed the payload as an image directly if you could execute the CSRF via GET request
3. Exploit stored XSS if you want to deliver CSRF to large audience
